...
def get_chunked_data(article_tokens: list, chunk_size_in_tokens: int, metadata: dict):
	...
	while len(article_tokens) != 0:
		chunk = {'chunk_tokens': [], 'chunk_ID': [], 'chunk_metadata': []}
		chunk['chunk_tokens'] = get_chunk(article_tokens, chunk_size_in_tokens, chunk_ID)
...
def get_chunk(article_tokens: list, chunk_size_in_tokens: int, current_chunk_ID: int):
